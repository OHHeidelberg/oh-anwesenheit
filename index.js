...
<style>
  ...
  .card { background: var(--card-bg); padding: 15px; border-radius: 20px; text-align: center; box-shadow: 0 8px 20px rgba(0,0,0,0.6); border: 1px solid #3d3d40; display: flex; flex-direction: column; align-items: center; }
...
app.get('/empfang', async (req, res) => {
    try {
        ...
        res.send(`<html>${htmlHead}<body class="empfang-body">${styles}<div class="container"><h1 class="empfang-header">Willkommen</h1>${infoBox}<div class="grid">${cards}</div></div></body></html>`);
