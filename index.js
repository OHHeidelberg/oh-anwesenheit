...
// Optimierter Emoji-Übersetzer inklusive Auto/Unterwegs
...
async function getSlackDetails(slackId) {
    if (!slackId) return { text: "ID fehlt", emoji: '❓', color: "bg-away", photo: "" };
    try {
        ...
        if (statusText) {
            ...
            else if (lower.includes("unterwegs") || lower.includes("fahrt")) color = "bg-away";
        } else if (isOnline) {
            color = "bg-active";
        }
    ...
    } catch (e) { return { text: "Fehler", emoji: '⚠️', color: "bg-away", photo: "" }; }
}
...
app.get('/dashboard', async (req, res) => {
    try {
        ...
        for (const row of rows) {
            ...
            cardsHtml += `
                <div class="card">
                    <img src="${status.photo || 'https://via.placeholder.com/100'}" class="avatar" onerror="this.src='https://via.placeholder.com/100'">
...
app.get('/', (req, res) => res.send('Server aktiv. <a href="/dashboard">Zum Dashboard</a>'));
