...
<style>
  ...
  .header-container { display: flex; align-items: center; justify-content: center; position: relative; width: 100%; padding: 15px 0; min-height: 40px; }
  .theme-toggle { position: absolute; right: 20px; top: 15px; background: var(--nav-btn-bg); border: 1px solid var(--border-color); color: var(--text-color); padding: 8px 12px; border-radius: 20px; cursor: pointer; font-size: 1.2rem; z-index: 10; }
  ...
  .tooltip { visibility: hidden; width: 160px; background-color: var(--nav-btn-bg); color: var(--text-color); border-radius: 12px; padding: 10px; position: absolute; z-index: 100; bottom: 110%; left: 50%; transform: translateX(-50%); box-shadow: 0 8px 16px rgba(0,0,0,0.2); font-size: 0.75rem; border: 1px solid var(--border-color); opacity: 0; transition: opacity 0.2s; pointer-events: none; text-align: left; }
...
function renderAvatar(person) {
    ...
    const content = `${hasPhoto ? `<img src="${person.p}" class="avatar" onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';"><div class="avatar-placeholder" style="display:none;">${firstLetter}</div>` : `<div class="avatar-placeholder">${firstLetter}</div>`}<div class="tooltip"><table><tr><th colspan="2">Kernarbeitszeit</th></tr>${tooltipRows}</table></div>`;
...
app.get('/dashboard', (req, res) => {
    ...
    res.send(`<html>${htmlHead}<body>${styles}
        <div class="header-container"><button class="theme-toggle" onclick="toggleTheme()">🌓</button></div>
        <div class="container">
            <div class="nav-bar">
...
app.get('/update', async (req, res) => {
    ...
    if (person?.id && person.id !== "kein") {
        ...
        try {
            await axios.post('https://slack.com/api/users.profile.set', { 
                user: person.id.trim(), 
                profile: { status_text: text, status_emoji: emoji, status_expiration: status === 'pause' ? 0 : expiration } 
            }, { headers: h });
            await updateData();
        } catch (e) {}
    }
